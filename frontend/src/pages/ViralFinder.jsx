...
export default function ViralFinder() {
  ...
  useEffect(() => {
    ...
    const fetchHistory = async () => {
      ...
      setHistoryError('');
      try {
        const response = await analysisApi.listRuns(user.id, { analysis_type: 'viral', limit: 5 });
        if (!cancelled) {
          setViralHistory(response.items || []);
        }
      } catch (err) {
        if (!cancelled) {
          setHistoryError(err.message || 'バイラル分析履歴の取得に失敗しました');
        }
      } finally {
        if (!cancelled) {
  ...
  return (
    <div>
      ...
      {/* Past Viral Analyses */}
      <div className="mb-8">
          <h2 className="text-xl font-bold text-gray-900 mb-4">過去のバイラル分析</h2>
          {historyLoading ? (
              ...
              <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 text-center">
                  <p className="text-gray-500">まだ保存されたバイラル分析はありません。</p>
              </div>
          ) : (
                  ...
                  {viralHistory.map((item) => (
                      <Link to={`/analysis/${item.id}`} key={item.id} className="block">
                          <div className="rounded-lg border border-gray-200 p-3 hover:bg-gray-50 transition-colors h-full flex flex-col">
                              ...
                              <span className="mt-3 inline-block text-xs text-yellow-500 hover:underline">詳細を見る</span>
                          </div>
                      </Link>
                  ))}
              </div>
